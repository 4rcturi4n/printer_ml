...
from printer_ml.kfold_X3DL_finetuned import (
    set_seed,
    train_x3d_finetune_fold,
)
...
CFG = {
    ...
    # output
    "out_dir": os.path.join(ROOT, "results", "x3d_finetune_saved_folds"),

    # k-fold
    "n_splits": 5,

    # model
    # Use "x3d_xs", "x3d_s", "x3d_m", or "x3d_l"
    "model_name": "x3d_l",
    "pretrained": True,

    # video sampling
    # Use "contiguous" if that worked better.
    # Use "uniform" for a fairer comparison against DINOv2.
    "sampling_mode": "contiguous",

    "clip_duration": 2.6,
    "num_frames": 16,

    # For x3d_xs you used 182 before.
    # X3D-L may need more GPU memory, so 224 is a safer first try.
    # If memory allows, you can try 312.
    "image_size": 312,

    # Single-clip fair setting:
    #     clips_train = 1
    #     clips_val = 1
    #
    # Stronger X3D video setting:
    #     clips_train = 3
    #     clips_val = 3
    "clips_train": 1,
    "clips_val": 1,
    ...
    # fine-tuning mode
    # Options:
    #   "none"          = frozen backbone, train head only
    #   "last_block"    = train head + last X3D block
    #   "last_n_blocks" = train head + last N X3D blocks
    #   "full"          = train all X3D backbone
    "finetune_mode": "last_block",
    "last_n_blocks": 1,

    # early stopping
    # Uses val_mae_phys, same as DINOv2
    ...
    # target transform
    # Same as DINOv2:
    # axial_resolution -> log -> normalize from train fold only
    "use_log_target": True,
...
def train_x3d_saved_folds():
    ...
    print(f"Fine-tune mode: {cfg['finetune_mode']}")
    print("Best checkpoint metric: val_mae_phys")
    print("Early stopping metric: val_mae_phys")
    ...
    for fold in range(1, cfg["n_splits"] + 1):
        ...
        history_df, fold_summary = train_x3d_finetune_fold(
            fold=fold,
            train_csv=train_csv,
            val_csv=val_csv,
    ...
    summary_csv_path = os.path.join(
        cfg["out_dir"],
        "x3d_finetune_saved_folds_summary.csv",
    )
    ...
    final_summary = {
        ...
        "early_stopping_metric": "val_mae_phys",
    }
    ...
    print()
    print("=" * 80)
    print("X3D FINE-TUNING ON SAVED FOLDS FINISHED")
    print("=" * 80)
